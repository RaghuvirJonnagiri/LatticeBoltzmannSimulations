...
# Plot Settings
Pinterval = 4000000# iterations to print next output
...
fin_g0 = cuda.mem_alloc(rho.size * rho.dtype.itemsize)
fin_g1 = cuda.mem_alloc(rho.size * rho.dtype.itemsize)
fin_g2 = cuda.mem_alloc(rho.size * rho.dtype.itemsize)
fin_g3 = cuda.mem_alloc(rho.size * rho.dtype.itemsize)
fin_g4 = cuda.mem_alloc(rho.size * rho.dtype.itemsize)
fin_g5 = cuda.mem_alloc(rho.size * rho.dtype.itemsize)
fin_g6 = cuda.mem_alloc(rho.size * rho.dtype.itemsize)
fin_g7 = cuda.mem_alloc(rho.size * rho.dtype.itemsize)
fin_g8 = cuda.mem_alloc(rho.size * rho.dtype.itemsize)


#ftemp_g = cuda.mem_alloc(fin.size * fin.dtype.itemsize)
feq_g0 = cuda.mem_alloc(rho.size * rho.dtype.itemsize)
feq_g1 = cuda.mem_alloc(rho.size * rho.dtype.itemsize)
feq_g2 = cuda.mem_alloc(rho.size * rho.dtype.itemsize)
feq_g3 = cuda.mem_alloc(rho.size * rho.dtype.itemsize)
feq_g4 = cuda.mem_alloc(rho.size * rho.dtype.itemsize)
feq_g5 = cuda.mem_alloc(rho.size * rho.dtype.itemsize)
...
cuda.memcpy_htod(fin_g0,fin[0]);cuda.memcpy_htod(fin_g1,fin[1]);cuda.memcpy_htod(fin_g2,fin[2])
cuda.memcpy_htod(fin_g3,fin[3]);cuda.memcpy_htod(fin_g4,fin[4]);cuda.memcpy_htod(fin_g5,fin[5])
cuda.memcpy_htod(fin_g6,fin[6]);cuda.memcpy_htod(fin_g7,fin[7]);cuda.memcpy_htod(fin_g8,fin[8])

cuda.memcpy_htod(feq_g0,fin[0]);cuda.memcpy_htod(feq_g1,fin[1]);cuda.memcpy_htod(feq_g2,fin[2])
cuda.memcpy_htod(feq_g3,fin[3]);cuda.memcpy_htod(feq_g4,fin[4]);cuda.memcpy_htod(feq_g5,fin[5])
cuda.memcpy_htod(feq_g6,fin[6]);cuda.memcpy_htod(feq_g7,fin[7]);cuda.memcpy_htod(feq_g8,fin[8])
...
mod = SourceModule ("""
    ...
    __global__ void funRT(float* fin_g0,float* fin_g1,float* fin_g2,float* fin_g3,float* fin_g4,float* fin_g5,float* fin_g6,float* fin_g7,float* fin_g8, 
                          ...
                          float* rho_g, float* u_g, float* taus_g){        int x     = threadIdx.x + blockIdx.x * blockDim.x;
        ...
        fin_l[0]=fin_g0[i];fin_l[1]=fin_g1[i];fin_l[2]=fin_g2[i];fin_l[3]=fin_g3[i];fin_l[4]=fin_g4[i];
        fin_l[5]=fin_g5[i];fin_l[6]=fin_g6[i];fin_l[7]=fin_g7[i];fin_l[8]=fin_g8[i];
        ...
        if ( (x>=0) and (x<xsize) and (y>=0) and (y<ysize) ){
            fin_g0[i] = fin_l[0] - omega*(fin_l[0]-feq_g0[i]) ; // j-c because -ve y axis  
        }
        ...
        if ( (x+1>=0) and (x+1<xsize) and (y>=0) and (y<ysize) ){
            fin_g1[i+1] = fin_l[1] - omega*(fin_l[1]-feq_g1[i]) ; // j-c because -ve y axis  
        }      
        cu[2] = (u_l[1]) ;      
        ...
        if ( (x>=0) and (x<xsize) and (y-1>=0) and (y-1<ysize) ){
            fin_g2[i-xsize] = fin_l[2] - omega*(fin_l[2]-feq_g2[i]) ; // j-c because -ve y axis  
        }        
        cu[3] = (-u_l[0]) ;      
        feq_g3[i] = rho_l*t_c[3]*(1. + 3.0*cu[3] + 9*0.5*cu[3]*cu[3] - 3.0*0.5*(u_l[0]*u_l[0] + u_l[1]*u_l[1]));
        if ( (x-1>=0) and (x-1<xsize) and (y>=0) and (y<ysize) ){
            fin_g3[i-1] = fin_l[3] - omega*(fin_l[3]-feq_g3[i]) ; // j-c because -ve y axis  
        }        
        cu[4] = (-u_l[1]) ;      
        feq_g4[i] = rho_l*t_c[4]*(1. + 3.0*cu[4] + 9*0.5*cu[4]*cu[4] - 3.0*0.5*(u_l[0]*u_l[0] + u_l[1]*u_l[1]));
        if ( (x>=0) and (x<xsize) and (y+1>=0) and (y+1<ysize) ){
            fin_g4[i+xsize] = fin_l[4] - omega*(fin_l[4]-feq_g4[i]) ; // j-c because -ve y axis  
        }
        ...
        if ( (x+1>=0) and (x+1<xsize) and (y-1>=0) and (y-1<ysize) ){
            fin_g5[i+1-xsize] = fin_l[5] - omega*(fin_l[5]-feq_g5[i]) ; // j-c because -ve y axis  
        }
                
        cu[6] = (-u_l[0] + u_l[1]) ;      
        feq_g6[i] = rho_l*t_c[6]*(1. + 3.0*cu[6] + 9*0.5*cu[6]*cu[6] - 3.0*0.5*(u_l[0]*u_l[0] + u_l[1]*u_l[1]));
        if ( (x-1>=0) and (x-1<xsize) and (y-1>=0) and (y-1<ysize) ){
            fin_g6[i-1-xsize] = fin_l[6] - omega*(fin_l[6]-feq_g6[i]) ; // j-c because -ve y axis  
        }
                
        cu[7] = (-u_l[0] -u_l[1]) ;      
        feq_g7[i] = rho_l*t_c[7]*(1. + 3.0*cu[7] + 9*0.5*cu[7]*cu[7] - 3.0*0.5*(u_l[0]*u_l[0] + u_l[1]*u_l[1]));
        if ( (x-1>=0) and (x-1<xsize) and (y+1>=0) and (y+1<ysize) ){
            fin_g7[i-1+xsize] = fin_l[7] - omega*(fin_l[7]-feq_g7[i]) ; // j-c because -ve y axis  
        }
                
        cu[8] = (u_l[0] -u_l[1]) ;      
        feq_g8[i] = rho_l*t_c[8]*(1. + 3.0*cu[8] + 9*0.5*cu[8]*cu[8] - 3.0*0.5*(u_l[0]*u_l[0] + u_l[1]*u_l[1]));
        if ( (x+1>=0) and (x+1<xsize) and (y+1>=0) and (y+1<ysize) ){
            fin_g8[i+1+xsize] = fin_l[8] - omega*(fin_l[8]-feq_g8[i]) ; // j-c because -ve y axis  
        }   

        
        if (x ==0 ){
            fin_g1[i] = feq_g1[i] - feq_g3[i] + fin_g3[i];
            fin_g5[i] = feq_g5[i] - feq_g7[i] + fin_g7[i];
            fin_g8[i] = feq_g8[i] - feq_g6[i] + fin_g6[i];
        }else if(x==xsize-1){
            fin_g3[i] = -feq_g1[i] + feq_g3[i] + fin_g1[i];
            fin_g6[i] = -feq_g8[i] + feq_g6[i] + fin_g8[i];
            fin_g7[i] = -feq_g5[i] + feq_g7[i] + fin_g5[i];
        }
                 
        if (y==ysize-1){   
            fin_g2[i] = -feq_g4[i] + feq_g2[i] + fin_g4[i];
            fin_g5[i] = -feq_g7[i] + feq_g5[i] + fin_g7[i];
            fin_g6[i] = -feq_g8[i] + feq_g6[i] + fin_g8[i];
        }else if (y==0){     
...
#    start.record() 
    ...
    funRT(fin_g0,fin_g1,fin_g2,fin_g3,fin_g4,fin_g5,fin_g6,fin_g7,fin_g8,
          feq_g0,feq_g1,feq_g2,feq_g3,feq_g4,feq_g5,feq_g6,feq_g7,feq_g8, 
          rho_g, u_g, taus_g, block=(blockDimX,blockDimY,1), grid=(gridDimX,gridDimY)) 
