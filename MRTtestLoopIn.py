...
#Number of cells
xsize = 32*15 # must be multiple of 32 for GPU processing
ysize = 32*15# must be multiple of 32 for GPU processing
...
GPUcode = """
    ...
    __host__ __device__ void dummy(void){
        //just testing;
        printf("blockcounter : %s \n",blockcounter); 
    }
        ...
        for ( it = 0; it<%(maxIt)s;it=it+1){
            ...
            for (k=0;k<9;k=k+1){
                if ( (x-c_c[k*2]>=0) and (x-c_c[k*2]<xsize) and (y+c_c[k*2+1]>=0) and (y+c_c[k*2+1]<ysize) ){
                    fin_g[k*d+i] = fin_g[k*d+x-c_c[k*2]+(y+c_c[k*2+1])*xsize];
                }
                __threadfence();
                __syncthreads();
                fin_l[k] = fin_g[k*d+i] ;
            ...
            if (x ==0 ){
                fin_l[1] = feq_g[1*d+i] - feq_g[3*d+i] + fin_g[3*d+i];
                fin_l[5] = feq_g[5*d+i] - feq_g[7*d+i] + fin_g[7*d+i];
                fin_l[8] = feq_g[8*d+i] - feq_g[6*d+i] + fin_g[6*d+i];
            }else if(x==xsize-1){
                fin_l[3] = -feq_g[1*d+i] + feq_g[3*d+i] + fin_g[1*d+i];
                fin_l[6] = -feq_g[8*d+i] + feq_g[6*d+i] + fin_g[8*d+i];
                fin_l[7] = -feq_g[5*d+i] + feq_g[7*d+i] + fin_g[5*d+i];
            }
                     
            if (y==ysize-1){   
                fin_l[2] = -feq_g[4*d+i] + feq_g[2*d+i] + fin_g[4*d+i];
                fin_l[5] = -feq_g[7*d+i] + feq_g[5*d+i] + fin_g[7*d+i];
                fin_l[6] = -feq_g[8*d+i] + feq_g[6*d+i] + fin_g[8*d+i];
            }else if (y==0){     
                fin_l[4] = -feq_g[2*d+i] + feq_g[4*d+i] + fin_g[2*d+i];
                fin_l[7] = -feq_g[5*d+i] + feq_g[7*d+i] + fin_g[5*d+i];
                fin_l[8] = -feq_g[6*d+i] + feq_g[8*d+i] + fin_g[6*d+i];
            } 
...
""" 
GPUcode = GPUcode % {'omega' : omega, 'maxIt' : maxIt}

# mod = JitLinkModule()
# mod.add_source(GPUcode, nvcc_options=['-O3', '-rdc=true', '-lcudadevrt'])
# mod.add_file(os.environ['CUDA_PATH'] + '\\lib\\x64\\cudadevrt.lib', jit_input_type.LIBRARY)
# mod.link()

#mod = SourceModule(GPUcode, options=['-rdc=true','-lcudart','-lcudadevrt','--machine=64'],arch='sm_60' )
