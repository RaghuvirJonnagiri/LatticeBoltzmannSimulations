...
velBCx_test = velBC[1,0,:,:].reshape(1,1,velBC.shape[-2],velBC.shape[-1])/np.max(velBC);
velBCy_test = velBC[1,1,:,:].reshape(1,1,velBC.shape[-2],velBC.shape[-1])/np.max(velBC);
...
Re_test = 100; 
Re_test_scaled = re_scaler.transform(np.array([Re_test]).reshape(1,1))[0,0]
...
vel_lbm = vel[np.array(np.where(Re==Re_test))[0]]
...
Re_test_scaled = Re_test_scaled.astype('float32')
fnet_test = np.empty((1,fnet.shape[1],fnet.shape[-2],fnet.shape[-1]))
vel_test = np.empty((1,vel.shape[1],vel.shape[-2],vel.shape[-1]))

Re = Re_scaled
fnet_test[0,:,:,:] = fnet[0,:,:,:]
fnet_test[0,-1,:,:] = Re_test_scaled

print("Scaled Re of "+str(Re_min)+':'+str(Re_max)+' to '+str(round(np.min(Re),3))+':'+str(np.max(Re)))
print("So, scaled Re_test is "+str(round(Re_test_scaled,3)))

for i in np.arange(feq.shape[0]):
    fnet[i,-1,:,:] = Re[i]  

modelx = load_model('cnn9_x.h5')
print('MODELX SUMMARY: ')
print(modelx.summary())
print('------------------------------------------------------')
modely = load_model('cnn10_y.h5', compile=False) #compile option is added to get around a bug
print('MODELY SUMMARY: ')
print(modely.summary())

velx = modelx.predict([fnet_test,velBCx_test,velBCy_test])   
vely = modely.predict([fnet_test,velBCx_test,velBCy_test])
...
#Re_test = Re_test*Re_max
vel_test[0,:] = vel_scaler.inverse_transform(vel_test[0,:])
vel_test[1,:] = vel_scaler.inverse_transform(vel_test[1,:])
uLB = vel_add
u = np.copy(vel_test)
...
print("Chosen Re to test is "+str(Re_test))
...
pyplot.savefig('CNN10_predict' + "_Re" + str(int(round(Re_test))) + ".png",bbox_inches = 'tight', pad_inches = 0.4)
