...
#optim = optimizers.RMSprop(lr=0.00005, rho=0.9, epsilon=1e-08, decay=0.001)
optim = optimizers.Adam(lr=0.00003, beta_1=0.9, beta_2=0.999, epsilon=1e-08, decay=0.007)
...
if xy ==0 :
    history = model.fit([x_train, velBCxtr, velBCytr], y_train[:,xy,:,:].reshape(int(0.8*vel.shape[0]),1,feq.shape[-2],feq.shape[-1]), epochs=10, batch_size=32,verbose=1,shuffle=True, validation_data=([x_test,velBCxte, velBCyte], y_test[:,xy,:,:].reshape(int(0.2*vel.shape[0]),1,feq.shape[-1],feq.shape[-1])))
elif xy == 1:
    history = model.fit([x_train, velBCxtr, velBCytr], y_train[:,xy,:,:].reshape(int(0.8*vel.shape[0]),1,feq.shape[-2],feq.shape[-1]), epochs=10, batch_size=32,verbose=1,validation_data=([x_test,velBCxte, velBCyte], y_test[:,xy,:,:].reshape(int(0.2*vel.shape[0]),1,feq.shape[-1],feq.shape[-1])))
