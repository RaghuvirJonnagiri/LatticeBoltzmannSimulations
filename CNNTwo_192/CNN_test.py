...
history = model.fit(x_train, y_train[:,xy,:,:].reshape((80,1,feq.shape[-1],feq.shape[-1])), epochs=1000, batch_size=5,verbose=1,validation_data=(x_test, y_test[:,xy,:,:].reshape((20,1,feq.shape[-1],feq.shape[-1]))))
pyplot.ioff()
