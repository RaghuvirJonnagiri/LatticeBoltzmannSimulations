    ...
    __global__ void funRT(float* fin_g, float* feq_g, float* rho_g, float* u_g, float* taus_g){
        ...
        for (k=0;k<9;k=k+1){
            ...
            if ( (x-c_c[k*2]>=0) and (x-c_c[k*2]<xsize) and (y+c_c[k*2+1]>=0) and (y+c_c[k*2+1]<ysize) ){
                fin_l[k] = fin_g[k*d+x-c_c[k*2]+(y+c_c[k*2+1])*xsize];
            ...
            //fin_l[k] = fin_g[k*d+i]*((x-c_c[k*2]<0) or (x-c_c[k*2]==xsize) or (y+c_c[k*2+1]<0) or (y+c_c[k*2+1]==ysize)) ;
            //fin_l[k] = fin_l[k] + fin_g[k*d+x-c_c[k*2]+(y+c_c[k*2+1])*xsize]*((x-c_c[k*2]>=0) and (x-c_c[k*2]<xsize) and (y+c_c[k*2+1]>=0) and (y+c_c[k*2+1]<ysize)) ;
        ...
        if (y==ysize-1){   
            fin_l[2] = -feq_g[4*d+i] + feq_g[2*d+i] + fin_l[4];
            fin_l[5] = -feq_g[7*d+i] + feq_g[5*d+i] + fin_l[7];
            fin_l[6] = -feq_g[8*d+i] + feq_g[6*d+i] + fin_l[8];
        }else if (y==0){     
            fin_l[4] = -feq_g[2*d+i] + feq_g[4*d+i] + fin_l[2];
            fin_l[7] = -feq_g[5*d+i] + feq_g[7*d+i] + fin_l[5];
            fin_l[8] = -feq_g[6*d+i] + feq_g[8*d+i] + fin_l[6];
        } 
        


        if (x ==0 ){
            fin_l[1] = feq_g[1*d+i] - feq_g[3*d+i] + fin_l[3];
            fin_l[5] = feq_g[5*d+i] - feq_g[7*d+i] + fin_l[7];
            fin_l[8] = feq_g[8*d+i] - feq_g[6*d+i] + fin_l[6];
        }else if(x==xsize-1){
            fin_l[3] = -feq_g[1*d+i] + feq_g[3*d+i] + fin_l[1];
            fin_l[6] = -feq_g[8*d+i] + feq_g[6*d+i] + fin_l[8];
            fin_l[7] = -feq_g[5*d+i] + feq_g[7*d+i] + fin_l[5];
        }
        ...
        /*
        if ( x == 0 or x == xsize-1 or y == ysize-1 or y==0){
  
            rho_l = rho_l*(y>0) + (y==0)*( fin_l[0]+fin_l[1]+fin_l[3]+2*(fin_l[2]+fin_l[5]+fin_l[6]) );
            u_l[0]= 0.08*(y==0) ; // uLB; 
            u_l[1] =0;
        }
        else {
        ...
        /*
        u_l[0] = ( x > 0 and x < xsize-1 and y>0 and y < ysize-1)*(c_c[0]*fin_l[0]+c_c[2]*fin_l[1]+c_c[4]*fin_l[2]+c_c[6]*fin_l[3]+c_c[8]*fin_l[4]+c_c[10]*fin_l[5]+c_c[12]*fin_l[6]+c_c[14]*fin_l[7]+c_c[16]*fin_l[8])/rho_l //continued
                 + (y==0)*0.08;
        u_l[1] = ( x > 0 and x < xsize-1 and y>0 and y < ysize-1)*(c_c[1]*fin_l[0]+c_c[3]*fin_l[1]+c_c[5]*fin_l[2]+c_c[7]*fin_l[3]+c_c[9]*fin_l[4]+c_c[11]*fin_l[5]+c_c[13]*fin_l[6]+c_c[15]*fin_l[7]+c_c[17]*fin_l[8])/rho_l;
        */   
        ...
        for (k=0;k<9;k=k+1){
            ...
            fin_g[k*d+i] = fin_l[k] - omega*(fin_l[k]-feq_g[k*d+i]) ; 
