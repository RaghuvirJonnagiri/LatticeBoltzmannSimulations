...
re_scaler = MinMaxScaler(feature_range=(0, 1))
...
history = model.fit(x_train, y_train[:,xy,:,:].reshape((80,1,feq.shape[-1],feq.shape[-1])), epochs=500, batch_size=20,verbose=1,validation_data=(x_test, y_test[:,xy,:,:].reshape((20,1,feq.shape[-1],feq.shape[-1]))))

f = pyplot.figure(figsize=(30,16))
pyplot.plot(history.history['loss']);pyplot.plot(history.history['val_loss'])
pyplot.title('model loss - cnn4_'+xydict[xy],fontsize = 40)
...
pyplot.savefig('cnn4_'+xydict[xy] +'_history' + ".png",bbox_inches = 'tight', pad_inches = 0.4)

model.save('cnn4_'+xydict[xy] +'.h5')
print('model saved as cnn4_'+xydict[xy] +'.h5')
