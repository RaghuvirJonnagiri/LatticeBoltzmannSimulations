...
model = load_model('cnn4_x.h5')
xy = 0 #choose 0 if x and 1 if y

xydict = {0:'x',1:'y'}

optim = optimizers.RMSprop(lr=0.0001, rho=0.9, epsilon=1e-08, decay=0.01)
...
history = model.fit(x_train, y_train[:,xy,:,:].reshape((80,1,feq.shape[-1],feq.shape[-1])), epochs=2000, batch_size=5,verbose=1,validation_data=(x_test, y_test[:,xy,:,:].reshape((20,1,feq.shape[-1],feq.shape[-1]))))
pyplot.ioff()
