...
Re = Re_scaled  # ignoring previous Re arrays and using a scikit scaled version
...
#optim = optimizers.Adadelta(lr=0.0005, rho=0.95, epsilon=1e-08, decay=0.01)
optim = optimizers.RMSprop(lr=0.0001, rho=0.9, epsilon=1e-08, decay=0.04)
#optim = optimizers.Nadam(lr=0.0005, beta_1=0.9, beta_2=0.999, epsilon=1e-08, schedule_decay=0.004)
...
if xy ==0 :
    history = model.fit([x_train, velBCxtr], y_train[:,xy,:,:].reshape(int(0.8*vel.shape[0]),1,feq.shape[-2],feq.shape[-1]), epochs=600, batch_size=20,verbose=1,validation_data=([x_test, velBCxte], y_test[:,xy,:,:].reshape(int(0.2*vel.shape[0]),1,feq.shape[-1],feq.shape[-1])))
elif xy == 1:
    history = model.fit([x_train, velBCytr], y_train[:,xy,:,:].reshape(int(0.8*vel.shape[0]),1,feq.shape[-2],feq.shape[-1]), epochs=600, batch_size=20,verbose=1,validation_data=([x_test, velBCyte], y_test[:,xy,:,:].reshape(int(0.2*vel.shape[0]),1,feq.shape[-1],feq.shape[-1])))
model.save('cnn8_'+xydict[xy] +'.h5')
print('model saved as cnn8_'+xydict[xy] +'.h5')

# #optim = optimizers.RMSprop(lr=0.001, rho=0.9, epsilon=1e-08, decay=0.01)
# optim = optimizers.Nadam(lr=0.0005, beta_1=0.9, beta_2=0.999, epsilon=1e-08, schedule_decay=0.004)
# model.compile(optimizer=optim, loss='mean_squared_error')
# history = model.fit(x_train, y_train[:,xy,:,:].reshape((80,1,feq.shape[-1],feq.shape[-1])), epochs=200, batch_size=5,verbose=1,validation_data=(x_test, y_test[:,xy,:,:].reshape((20,1,feq.shape[-1],feq.shape[-1]))))
# model.save('cnn7_'+xydict[xy] +'.h5')
# print('model saved as cnn7_'+xydict[xy] +'.h5')
# 
# 
# optim = optimizers.RMSprop(lr=0.00001, rho=0.9, epsilon=1e-08, decay=0.01)
# model.compile(optimizer=optim, loss='mean_squared_error')
# history = model.fit(x_train, y_train[:,xy,:,:].reshape((80,1,feq.shape[-1],feq.shape[-1])), epochs=100, batch_size=5,verbose=1,validation_data=(x_test, y_test[:,xy,:,:].reshape((20,1,feq.shape[-1],feq.shape[-1]))))

#history = model.fit(x_train, y_train[:,xy,:,:].reshape((80,1,feq.shape[-1],feq.shape[-1])), epochs=400, batch_size=5,verbose=1,validation_data=(x_test, y_test[:,xy,:,:].reshape((20,1,feq.shape[-1],feq.shape[-1]))))
#history = model.fit(x_train, y_train[:,xy,:,:].reshape((80,1,feq.shape[-1],feq.shape[-1])), epochs=500, batch_size=5,verbose=1,validation_data=(x_test, y_test[:,xy,:,:].reshape((20,1,feq.shape[-1],feq.shape[-1]))))
#history = model.fit(x_train, y_train[:,xy,:,:].reshape((80,1,feq.shape[-1],feq.shape[-1])), epochs=500, batch_size=5,verbose=1,validation_data=(x_test, y_test[:,xy,:,:].reshape((20,1,feq.shape[-1],feq.shape[-1]))))

pyplot.ioff()
f = pyplot.figure(figsize=(30,16))
pyplot.plot(history.history['loss']);pyplot.plot(history.history['val_loss'])
pyplot.title('model loss - cnn8_'+xydict[xy],fontsize = 40)
...
pyplot.savefig('cnn8_'+xydict[xy] +'_history' + ".png",bbox_inches = 'tight', pad_inches = 0.4)
