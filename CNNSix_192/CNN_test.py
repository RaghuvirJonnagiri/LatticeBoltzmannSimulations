...
optim = optimizers.RMSprop(lr=0.00002, rho=0.9, epsilon=1e-08, decay=0.01)
...
history = model.fit(x_train, y_train[:,xy,:,:].reshape((80,1,feq.shape[-1],feq.shape[-1])), epochs=300, batch_size=5,verbose=1,validation_data=(x_test, y_test[:,xy,:,:].reshape((20,1,feq.shape[-1],feq.shape[-1]))))
...
pyplot.title('model loss - cnn6_'+xydict[xy],fontsize = 40)
...
pyplot.savefig('cnn6_'+xydict[xy] +'_history' + ".png",bbox_inches = 'tight', pad_inches = 0.4)

model.save('cnn6_'+xydict[xy] +'.h5')
print('model saved as cnn6_'+xydict[xy] +'.h5')
