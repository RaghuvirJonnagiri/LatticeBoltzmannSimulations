...
# Plot Settings
Pinterval = 300000# iterations to print next output
...
c_h = np.zeros((18), dtype=float).astype(np.float32)
...
c_h[0]=0.0;c_h[1]=0.0;c_h[2]=1.0;c_h[3]=0.0;c_h[4]=0.0;c_h[5]=1.0;c_h[6]=-1.0;c_h[7]=0.0;c_h[8]=0.0;
c_h[9]=-1.0;c_h[10]=1.0;c_h[11]=1.0;c_h[12]=-1.0;c_h[13]=1.0;c_h[14]=-1.0;c_h[15]=-1.0;c_h[16]=1.0;c_h[17]=-1.0;
...
if RT == 'SRT':
    ...
    funRT = """
        __global__ void funRT(float* fin_g, float* feq_g, float* rho_g, float* u_g, float* taus_g){
            ...
            //t_c[0]=4.0/9.0 ;t_c[1]=1.0/9.0;t_c[2]=1.0/9.0;t_c[3]=1.0/9.0;t_c[4]=1.0/9.0;t_c[5]=1.0/36.0;t_c[6]=1.0/36.0;t_c[7]=1.0/36.0;t_c[8]=1.0/36.0;
            //c_c[0]=0;c_c[1]=0;c_c[2]=1;c_c[3]=0;c_c[4]=0;c_c[5]=1;c_c[6]=-1;c_c[7]=0;c_c[8]=0;
            //c_c[9]=-1;c_c[10]=1;c_c[11]=1;c_c[12]=-1;c_c[13]=1;c_c[14]=-1;c_c[15]=-1;c_c[16]=1;c_c[17]=-1;
            ...
            if (turb==1) {
                ...
                for (k=0;k<9;k=k+1){
                    product1 = c_c[k*2]*c_c[k*2+1]*fin_l[k] + product1; 
                    product2 = c_c[k*2]*c_c[k*2+1]*feq_g[k*d+i] + product2;  
                }
            ...
            u_g[0*d+i] = (c_c[0]*fin_l[0]+c_c[2]*fin_l[1]+c_c[4]*fin_l[2]+c_c[6]*fin_l[3]+c_c[8]*fin_l[4]+c_c[10]*fin_l[5]+c_c[12]*fin_l[6]+c_c[14]*fin_l[7]+c_c[16]*fin_l[8])/rho_g[i];
            u_g[1*d+i] = (c_c[1]*fin_l[0]+c_c[3]*fin_l[1]+c_c[5]*fin_l[2]+c_c[7]*fin_l[3]+c_c[9]*fin_l[4]+c_c[11]*fin_l[5]+c_c[13]*fin_l[6]+c_c[15]*fin_l[7]+c_c[17]*fin_l[8])/rho_g[i];
            ...
            __syncthreads();
            for (k=0;k<9;k=k+1){ 
                cu = (c_c[k*2]*u_g[0*d+i] + c_c[k*2+1]*u_g[1*d+i]) ;      
                feq_g[k*d+i] = rho_l*t_c[k]*(1. + 3.0*cu + 9*0.5*cu*cu - 3.0*0.5*usqr);
            ...
            //fin_g[0*d+i] = ftemp_g[0*d+i];fin_g[1*d+i] = ftemp_g[1*d+i];fin_g[2*d+i] = ftemp_g[2*d+i];
            //fin_g[3*d+i] = ftemp_g[3*d+i];fin_g[4*d+i] = ftemp_g[4*d+i];fin_g[5*d+i] = ftemp_g[5*d+i];
            //fin_g[6*d+i] = ftemp_g[6*d+i];fin_g[7*d+i] = ftemp_g[7*d+i];fin_g[8*d+i] = ftemp_g[8*d+i];
